...
const authContract = c.router({
    signUp: {
        path: '/auth/sign-up',
        method: 'POST',
        body: z.object({ email: z.string().email(), password: z.string().min(8).max(50), username: z.string().min(3).max(20) }),
        responses: {
            201: tokensSchema,
    ...
    signIn: {
        ...
        responses: {
            200: tokensSchema,
            400: z.object({ message: z.enum([USER_API_ERRORS.unknownUser, USER_API_ERRORS.wrongPassword]) }),
        },
    },
    refreshTokens: {
        ...
        responses: {
            200: tokensSchema,
            400: z.object({
                message: z.enum([USER_API_ERRORS.invalidRefreshToken, COMMON_API_ERRORS.NOT_FOUND, USER_API_ERRORS.missingRefreshToken]),
            }),
        },
    },
});
...
const userContract = c.router({
    getMe: {
        ...
        responses: {
            200: z.object({ username: z.string(), email: z.string(), id: z.number() }),

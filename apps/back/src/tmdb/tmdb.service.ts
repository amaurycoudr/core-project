...
import type { Gender, Job } from "src/movies/movies.schema";
import { z } from "zod";
...
export class TmdbService {
	...
	async search(query: string, { page }: { page?: number }) {
		...
		return z
			.object({
				data: z.object({
					results: z.array(this.tmdbSearchMovie),
	...
	private tmdbMovieCreditSchema = () =>
		z.object({
			...
			crew: z.array(
				z.object({ id: z.number(), job: z.string() }).transform(({ id, job }) => ({
					tmdbId: id,
					job: this.getJobFromTmdb(job),
				})),
			),
		});
	private tmdbMovieDetailsSchema = z
		.object({

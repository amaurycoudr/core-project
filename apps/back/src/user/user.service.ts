...
import { UNIQUE_CONSTRAINT, lower } from "src/drizzle/drizzle.helper";
import { DrizzleService } from "src/drizzle/drizzle.service";
import { type CreateUser, users } from "./schemas/user.schema";
	...
	async create({ email, username, password }: CreateUser) {
		try {
			...
			if ((error as Record<string, unknown>).code === UNIQUE_CONSTRAINT) {
				return {
					data: USER_API_ERRORS.emailAlreadyUsed,
					result: ERROR,
				} as const;
			}
			throw error;
